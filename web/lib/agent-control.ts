...
const actions = new Set(["start", "pause", "restart"]);
...
export async function ensureAgentTables() {
  ready ??= query(`CREATE TABLE IF NOT EXISTS public.mix_agents (
  ...
  CREATE INDEX IF NOT EXISTS mix_agent_commands_pending_idx ON public.mix_agent_commands(agent_id,status,id);`).then(() => undefined).catch((error) => {
    ready = null;
    throw error;
  });
  return ready;
...
export async function registerAgent(input: Record<string, unknown>, mixBearer: string) {
  ...
  const row = await query<{ id: string }>(`INSERT INTO public.mix_agents
    (id,owner_id,cnpj,machine_id,computer_name,windows_user,integrator_path,agent_version,device_secret_hash,desired_state,last_seen)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'running',NULL)
    ...
    RETURNING id`, [id, ownerId, cnpj, machineId, computerName, windowsUser, integratorPath, agentVersion, hashSecret(secret)]);
...
export async function acceptHeartbeat(agent: AgentRow, input: Record<string, unknown>) {
  await ensureAgentTables();
  const online = Boolean(input.integrator_online);
  const session = Boolean(input.session_ready);
  ...
  const updated = await transaction(async (client) => {
    const state = await client.query<{ desired_state: "running" | "paused" }>(
      `UPDATE public.mix_agents SET integrator_online=$1,session_ready=$2,agent_version=$3,last_seen=NOW(),updated_at=NOW()
       WHERE id=$4 RETURNING desired_state`, [online, session, version, agent.id],
    );
    const command = await client.query<{ id: string; action: string }>(
      `SELECT id,action FROM public.mix_agent_commands
       WHERE agent_id=$1 AND (status='pending' OR (status='delivered' AND delivered_at<NOW()-INTERVAL '2 minutes'))
...
export async function completeCommand(agent: AgentRow, input: Record<string, unknown>) {
  ...
  const result = await query(`UPDATE public.mix_agent_commands SET status=$1,success=$2,result_message=$3,completed_at=NOW()
    WHERE id=$4 AND agent_id=$5 RETURNING id`, [success ? "completed" : "failed", success, message, commandId, agent.id]);
  if (!result.rowCount) throw new Error("Comando nao encontrado para este Agente.");
  return { ok: true };
}
