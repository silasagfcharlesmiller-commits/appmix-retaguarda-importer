...
func fileVersion(path string) string {
	...
	script := `[Diagnostics.FileVersionInfo]::GetVersionInfo('` + strings.ReplaceAll(path, "'", "''") + `').FileVersion`
	output, err := hiddenCommand("powershell.exe", "-NoProfile", "-NonInteractive", "-EncodedCommand", encodedPowerShell(script)).Output()
...
func securityProtectionFindings(targetDir string) int {
	script := `$target=[IO.Path]::GetFullPath('` + strings.ReplaceAll(targetDir, "'", "''") + `');$result=@();if(Get-Command Get-MpThreatDetection -ErrorAction SilentlyContinue){$result=@(Get-MpThreatDetection -ErrorAction SilentlyContinue|Where-Object{($_.Resources -join ' ') -like ('*'+$target+'*') -or ($_.Resources -join ' ') -like '*desktop-integrador.exe*'}|Select-Object -First 10 ThreatID,InitialDetectionTime,ActionSuccess,Resources)};$result|ConvertTo-Json -Depth 4 -Compress`
	output, err := hiddenCommand("powershell.exe", "-NoProfile", "-NonInteractive", "-EncodedCommand", encodedPowerShell(script)).Output()
	if err != nil || strings.TrimSpace(string(output)) == "" {
		return 0
	}
...
func (installer *Installer) EnvironmentReport(diagnostics *Diagnostics) (EnvironmentReport, error) {
	identity, checks := environmentChecks(installer.TargetDir, diagnostics, true)
	...
	report := EnvironmentReport{
		Ready: blockingChecks(checks) == nil, Checks: checks,
		...
		ProfileReady:          checks[2].Status == "ok",
		TaskScheduler:         "disponível",
	}
	if manifest, manifestErr := RemoteVersionManifest(5 * time.Second); manifestErr == nil {
